...
código y dudas), movidas acá el 2026-07-24 para mantener liviana la guía
activa (GUIA-JAVA.md). Se consulta SOLO para:
- RE-ESTUDIO (cuando un repaso sale mal dos veces seguidas), o
- cuando el usuario pide releer una sesión vieja puntual.
Las sesiones #30 en adelante viven en GUIA-JAVA.md en formato corto.
...
SESIÓN #26 — 2026-07-24 — Varios objetos de una clase + "salí de main": Objectville de verdad (Ubicación pág. 170-173 de 1629, 12%)

IDEAS CLAVE

- Ejemplo `Movie`/`MovieTestDrive`: se crean TRES objetos de la misma
  clase (`one`, `two`, `three`), cada uno con sus PROPIOS valores de
  `title`/`genre`/`rating` — confirma que cada objeto guarda su propia
  copia de las instance variables, aunque compartan el mismo plano
  (clase). `playIt()` solo se llama sobre `two`.
- Los tres títulos son chistes con películas reales en inglés (el
  libro no lo explica): "Gone with the Stock" (de "Gone with the
  Wind"/"Lo que el viento se llevó", clásico dramático de 1939),
  "Lost in Cubicle Space" (de "Lost in Space"/"Perdidos en el
  espacio", ciencia ficción), "Byte Club" (de "Fight Club"/"El club de
  la pelea", 1999). Cada uno coherente con su género y puntaje.
- "Sharpen your pencil: Movie objects" (pág. 172-173): ejercicio de
  LECTURA (sin código para escribir) — completar una tabla con los
  valores finales de los tres objetos al terminar `main()`.
- "Quick! Get out of main!": mientras TODO el trabajo pase dentro de
  un único `main()` (patrón TestDrive que venimos usando), no se está
  en "Objectville" de verdad — nombre de broma del libro para el
  mundo real de la POO, donde los OBJETOS hablan entre sí (un objeto
  llama methods de otro objeto) en vez de que un solo método haga
  todo.
- Dos usos de `main()`: (1) probar una clase real (patrón TestDrive
  que ya conocemos), (2) arrancar/lanzar una aplicación Java real.
  Capítulo 4 sigue usando el patrón (1); Capítulo 6 muestra el uso (2).
- Adelanto conceptual "The Guessing Game": un objeto `GuessGame` y
  tres objetos `Player`. Tres clases con responsabilidades separadas:
  `GameLauncher` (tiene el `main()`, arranca la app creando UN objeto
  `GuessGame` y llamando a `startGame()`), `GuessGame` (su
  `startGame()` crea los tres `Player`, piensa un número al azar entre
  0 y 9, les pide que adivinen y anuncia el resultado), `Player`. Es
  el contraste real con `MovieTestDrive`: acá `main()` no hace todo el
  trabajo, solo arranca y DELEGA a otro objeto, que a su vez coordina
  a más objetos. Código real de estas tres clases todavía no se
  mostró, la página corta ahí.
...
- "Sharpen your pencil: Movie objects" (pág. 172-173) — pendiente, ver
  EJERCICIOS.md. Archivo de arranque en
  ejercicios/lib04-movie-objects.md.

NOTA DEL PROFE (cosas que el libro no dice pero importan)

- "Objectville" es un nombre humorístico del libro, no un término
  técnico real — no hace falta buscarlo en documentación de Java.
- La distinción "main() prueba vs. main() arranca la app real" es la
  semilla de un tema que se profundiza recién en Capítulos 4 y 6 — por
  ahora alcanza con tenerla identificada, no hace falta dominarla.

DUDAS QUE SURGIERON (y su respuesta corta)

- Ronda de comprensión: P1 (`Movie a = new Movie(); Movie b = new
  Movie(); a.rating = 10;` — ¿cuánto vale `b.rating`?) respondida
  INCORRECTA: dijo `null`. Corrección: `rating` es `int` (tipo
  primitivo), y los primitivos numéricos tienen su PROPIO valor por
  defecto (`0` para numéricos, `false` para boolean) — `null` es solo
  para tipos de referencia (objetos, `String`) que nunca recibieron un
  objeto asignado. `b.rating` vale `0`, no `null`. P2 (diferencia entre
  que `main()` haga todo vs. que los objetos se hablen entre sí)
  respondida CORRECTA y completa: identificó delegación, cada clase
  con un propósito acotado, y por qué concentrar todo en `main()` se
  vuelve frágil a medida que crece el código.

EJERCICIOS CREADOS: ninguno
PRÓXIMO PASO: seguir con el código real de "The Guessing Game"
(GuessGame, Player, GameLauncher), que todavía no se mostró.

SESIÓN #27 — 2026-07-24 — The Guessing Game: código real, Math.random(), while(true)/break, el heap y garbage collection (Ubicación pág. 175-178 de 1629, 12%)

IDEAS CLAVE

- Diagrama faux-UML de las 3 clases del juego (pág. 175): `GameLauncher`
  (sin instance variables, solo `main()`, crea un `GuessGame` y llama a
  `startGame()`), `GuessGame` (instance variables `p1`/`p2`/`p3` de
  tipo `Player`, method `startGame()`), `Player` (instance variable
  `number`, method `guess()`). Confirma en código real el patrón
  anticipado en la Sesión #26: `main()` no hace todo, delega.
- Código completo de `GuessGame.startGame()` (pág. 176): crea los 3
  `Player`, arma un número secreto al azar, y en un bucle les pide que
  adivinen hasta que alguien acierte.
- SINTAXIS NUEVA #1 — `Math.random()`: se llama SIN `new` porque es un
  METHOD ESTÁTICO (static method), pertenece a la clase `Math` en sí
  misma, no a un objeto. Devuelve un `double` entre 0.0 (incluido) y
  1.0 (excluido). `(int) (Math.random() * 10)` usa un CAST (`(int)`)
  para truncar (no redondear) el decimal y quedarse con un entero entre
  0 y 9 — alternativa a `java.util.Random`/`nextInt()` (Sesión #15).
- SINTAXIS NUEVA #2 — `while (true)` + `break`: un `while` cuya
  condición nunca es falsa, diseñado para repetir "hasta que algo pase
  adentro" en vez de "hasta que una condición del encabezado se vuelva
  false". La instrucción `break` corta el bucle de inmediato apenas se
  ejecuta, sin importar la condición del `while`.
- SINTAXIS NUEVA #3 — operador `||` (OR/"o"): `if (p1isRight ||
  p2isRight || p3isRight)` da `true` si AL MENOS UNO de los tres es
  `true`. Solo da `false` si los tres son `false`.
- "Java takes out the garbage" (pág. 176): todo objeto creado con
  `new` vive en un área de memoria llamada "the heap" (el heap, más
  precisamente "Garbage-Collectible Heap"). Java reserva ahí el
  espacio que cada objeto necesita. Cuando la JVM detecta que un
  objeto ya no puede usarse (nada lo referencia), queda "eligible for
  garbage collection"; si hace falta memoria, el Garbage Collector lo
  descarta y libera ese espacio — el programador nunca libera memoria
  a mano en Java.
- Salida real de `java GameLauncher` (pág. 178): confirma el patrón
  `while(true)`/`break` en acción — dos vueltas sin ganador ("Players
  will have to try again") y en la tercera el jugador 1 acierta,
  dispara el `if` con el `break`, termina el juego.
...
- Ninguno; tanda puramente conceptual/de código de ejemplo.

NOTA DEL PROFE (cosas que el libro no dice pero importan)

- El código de la clase `Player` (su method `guess()`) NO se mostró en
  esta tanda, solo su diagrama — se infiere por la salida ("I'm
  guessing X") que genera un número al azar, lo imprime y lo guarda en
  su propia instance variable `number`. Pendiente ver su código real.
- Se salteó la página 177 (el pantallazo saltó de la 176 a la 178) —
  avisado al usuario; no pareció bloquear la comprensión del flujo.
- `static method` es un adelanto informal (como fue "instance
  variable" antes de su nombre formal) — se profundiza en capítulos
  futuros del libro, no hace falta dominarlo todavía, solo reconocer
  la señal `NombreClase.metodo()` sin `new`.

DUDAS QUE SURGIERON (y su respuesta corta)

- Ronda de comprensión: P1 (¿por qué `Math.random()` no necesita `new
  Math()`?) respondida con una imprecisión de término: dijo "porque es
  una clase estática". Corrección: lo estático es el METHOD `random()`,
  no la clase `Math` (una clase de nivel superior ni siquiera puede
  declararse `static` en Java, eso solo aplica a clases anidadas). El
  razonamiento de fondo (estático → no hace falta instancia) estaba
  bien, solo mal aplicado a la clase en vez de al method. P2 (¿qué
  pasaría si se sacara el `break;`?) respondida CORRECTA: el juego
  nunca terminaría, bucle infinito. Bonus explicado en el chat: como
  las variables `pXisRight` nunca se resetean a `false`, sin el
  `break` el programa imprimiría "We have a winner!" en TODAS las
  vueltas siguientes una vez que alguien acierta una vez.

EJERCICIOS CREADOS: ninguno
PRÓXIMO PASO: ver el código real de la clase `Player` (pendiente,
saltada la página 177) y seguir la siguiente tanda del libro.

SESIÓN #28 — 2026-07-24 — Cierre del Capítulo 2: static como excepción, JAR/manifest, resumen y "BE the Compiler" parte 2 (Ubicación pág. 179-184 de 1629, 12%)

IDEAS CLAVE

- Q&A de cierre sobre `static` (pág. 179): en Java no existe "global"
  tal cual, pero un method `public static` se comporta como uno (ej.
  `Math.random()`); una variable `public static final` es una
  constante realmente global (adelanto formal en Capítulo 10). Aun
  así, `static` sigue viviendo DENTRO de una clase (ej. `pi` y
  `random()` están definidos dentro de la clase `Math`) — y es la
  EXCEPCIÓN, no la regla: representa el caso especial donde a
  propósito no hay múltiples instancias/objetos.
- Qué ES un programa Java: un montón de clases (al menos una con
  `main()`). Para distribuirlo a usuarios sin JVM, hay que incluir la
  JVM junto con las clases. Para no entregar cientos de archivos
  sueltos, se empaquetan en un **JAR** (Java ARchive, basado en
  pkzip), que puede incluir un **manifest** indicando qué clase tiene
  el `main()` a ejecutar.
- "Make it Stick" (pág. 181): nueva analogía — una clase es como una
  RECETA, los objetos son como GALLETITAS hechas con ella (mismo
  concepto que "plano/blueprint", reforzado con otra imagen). Aparecen
  también puros adelantos sin explicar todavía (pass by value,
  threads/wait()/notify()) y un chiste visual ("Wash Cat") sobre cómo
  el cerebro memoriza fácil lo cotidiano y le cuesta lo técnico.
- CIERRE DEL CAPÍTULO 2 (pág. 182, "Bullet Points"): resumen de todo
  lo visto desde la Sesión #19 — POO permite extender sin tocar código
  ya probado; todo código vive en una clase; una clase es un plano/
  receta; un objeto se encarga de sí mismo (no importa cómo lo hace);
  un objeto SABE cosas (instance variables, estado) y HACE cosas
  (methods, comportamiento); conviene una clase TestDrive separada
  para probar; una clase puede heredar de una superclase; en runtime,
  un programa Java es solo objetos hablando entre sí.
- Arranca la sección de ejercicios de cierre de capítulo con "BE the
  Compiler, parte 2" (pág. 183-184): dos archivos nuevos para decidir
  si compilan (mismo formato que la Sesión #16).

CÓDIGO CLAVE (el Java esencial de la tanda, ya explicado en el chat)

- El código del ejercicio "BE the Compiler, parte 2" está en
  EJERCICIOS.md / ejercicios/lib05-be-the-compiler-2.md (no se repite
  acá para no adelantar la solución antes de que el usuario lo intente).

EJERCICIOS DEL LIBRO EN ESTA TANDA (nombre + hecho/pendiente)

- "BE the Compiler, parte 2" (pág. 183-184) — pendiente, ver
  EJERCICIOS.md. Archivo de arranque en
  ejercicios/lib05-be-the-compiler-2.md.

NOTA DEL PROFE (cosas que el libro no dice pero importan)

- El patrón `public static final` para constantes es EXACTAMENTE lo
  que vas a usar en el mundo real para cosas como valores de
  configuración compartidos — vale la pena tenerlo identificado ya,
  aunque recién se formalice en el Capítulo 10.
- Los archivos JAR son la base de cómo se empaquetan y distribuyen las
  apps Java reales (y las librerías que vas a usar en el trabajo) —
  es contexto útil, no hace falta dominarlo todavía.

DUDAS QUE SURGIERON (y su respuesta corta)

- Ronda de comprensión: P1 (¿por qué `static` es "la excepción, no la
  regla" en POO?) respondida con duda genuina, sin term inar de decidir
  entre dos lecturas — ninguna de las dos era exacta. Aclarado: no es
  que la POO en general prohíba lo estático (la mayoría de lenguajes
  OO lo soportan) ni que sea una rareza exclusiva de Java. Es sobre el
  espíritu de diseño: la POO organiza datos y comportamiento DENTRO de
  objetos/instancias; `static` pertenece a la clase, no a ningún
  objeto, así que va un poco en contra de esa idea — existe como
  válvula de escape intencional para los pocos casos sin necesidad de
  múltiples instancias (utilidades, constantes reales). "Excepción, no
  regla" habla de DISEÑO (la mayoría de tus clases/methods NO deberían
  ser static), no de que la sintaxis sea rara de ver. P2 (con la
  analogía receta/galletita: ¿qué sería `r` de `new Robot()`?)
  respondida CORRECTA: `r` es una galletita hecha con la receta Robot.

EJERCICIOS CREADOS: ninguno
PRÓXIMO PASO: resolver "BE the Compiler, parte 2" y seguir con el
resto del cierre del Capítulo 2 (más ejercicios antes de arrancar el
Capítulo 3).

SESIÓN #29 — 2026-07-24 — Más ejercicios de cierre del Capítulo 2: Code Magnets "DrumKit" y arranca Pool Puzzle "Echo" (Ubicación pág. 186-190 de 1629, 13%)

IDEAS CLAVE

- Code Magnets "DrumKit" (pág. 186): mismo formato que "Shuffle1"
  (Sesión #15) — imanes desordenados de dos clases (`DrumKit` con
  instance variables booleanas `topHat`/`snare` y methods
  `playSnare()`/`playTopHat()`; `DrumKitTestDrive` con el `main()`)
  para reordenar y lograr una salida exacta. Se creó el EJERCICIO #06.
- Arranca un "Pool Puzzle" nuevo (pág. 187-190), esta vez con el
  programa "Echo": mismo tipo de acertijo que el EJERCICIO #04
  (Sesión #17), completar espacios en blanco con fragmentos de "la
  pileta" (reutilizables), para que `EchoTestDrive` compile y dé una
  salida exacta.
- La imagen de "la pileta" (fragmentos disponibles) llegó en una
  captura aparte, también de la página 190: nombres (`x`, `y`, `e2`,
  `count`, `Echo`, `Tester`, `echo()`, `count()`, `hello()`),
  condiciones (`x<4`, `x<5`, `x>0`, `x>1`, `x==3`, `x==4`) y
  asignaciones (`e1=e1+1;`, `e2=e1;`, `Echo e2 = new Echo();`, etc.).
  A diferencia del EJERCICIO #04, acá el libro SÍ permite repetir
  fragmentos. Se creó el EJERCICIO #07 con el acertijo completo.

CÓDIGO CLAVE (el Java esencial de la tanda, ya explicado en el chat)

- El código de "DrumKit" está en EJERCICIOS.md /
  ejercicios/ej06-drumkit/ (no se repite acá para no adelantar la
  solución). El código incompleto de "Echo"/"EchoTestDrive" (con
  huecos) está en el chat de esta sesión, pendiente de registrar como
  ejercicio formal hasta tener la página 188.

EJERCICIOS DEL LIBRO EN ESTA TANDA (nombre + hecho/pendiente)

- "Code Magnets: DrumKit" (pág. 186) — pendiente, ver EJERCICIOS.md,
  EJERCICIO #06.
- "Pool Puzzle: Echo" (pág. 187-190) — pendiente, ver EJERCICIOS.md,
  EJERCICIO #07. Archivo de arranque en
  ejercicios/ej07-pool-puzzle-echo/EchoTestDrive.java.

NOTA DEL PROFE (cosas que el libro no dice pero importan)

- Ninguna nueva esta tanda (tanda mayormente de ejercicios de
  práctica, sin teoría nueva).

DUDAS QUE SURGIERON (y su respuesta corta)

- Ninguna todavía; ronda de comprensión pendiente hasta cerrar el
  Pool Puzzle "Echo" con la página que falta.

EJERCICIOS CREADOS: #06 DrumKit (Code Magnets)
