...
- PRÓXIMA SESIÓN: `/rename java-s105` (sale SIEMPRE de esta línea, NO se calcula: es un contador distinto al de las tandas. La última fue java-s104: solo arranque, sin trabajo — el usuario siguió en el otro PC. Antes: java-s103, repaso "compilador vs. JVM" r1.) ⚠ Los 9 pantallazos del cap. 8 (pág. 541+, del 21/09 y 24/09) quedaron en `paginas/` del PC de escritorio: NO viajan por git.
...
- ⚠ **21 repasos en cola** (el más viejo, lib11 "Mixed Messages" r1, del 2026-08-02). El de "qué método corre cuando hay sobrescritura" r1 salió **BIEN el 23/09** (5/5: leyó el método entero y respetó el orden — las dos caídas de los exámenes quedaron resueltas); r2 al 2026-10-07. El gemelo, CONCEPTO "compilador vs. JVM" r1, salió **PARCIAL el 24/09**: ya dice "la JVM elige" y "no compila", pero cree que el compilador mira la clase del OBJETO (propuso agregar el método en la subclase para que compile); r1 bis al 2026-09-27 ⚠ PRIORIDAD, si falla es RE-ESTUDIO. La cola NO converge: entran ~1,3/día y sale 1. Se atacan INDIVIDUALES y por RIESGO, no por fecha; el arranque lo crea `/repaso`. lib23 (árbol Doctor) r1 el 18/09: PARCIAL 6/8 — contó `makesHouseCalls` como método por no mirar los paréntesis del diagrama; r1 bis al 2026-09-21, si falla otra vez es RE-ESTUDIO. Errores a vigilar: contestar solo la PRIMERA mitad de cada pregunta (ToDo/entregar-un-ejercicio.md), decir "no se ejecuta" cuando el programa arranca y revienta, no nombrar la excepción, llamar "lista" a un arreglo, y hardcodear el tamaño en vez de `.length`.
...
- Último triage (`/pendientes`): **2026-09-23** — PENDIENTES.md queda con UNA entrada viva: el Ready-Bake de GameHelper. Los 4 huecos de formato (447-448, 479-480, 483, 502-503) se dieron de baja: son tramos de los capítulos 6 y 7, ya cerrados. Ejercicios: ninguno pasa las 3 semanas, no hubo bajas. Ritmo real: **5,45 pág./tanda** (99 tandas, pág. 540) → faltan **~200 tandas**; con el material como TEXTO en vez de pantallazos bajarían a ~145. ⚠ La cola de REPASOS no converge: entran ~1,3 por día y sale 1 como máximo (16 el 19/09 → 22 hoy).
- Último examen (`/examen`): **2026-09-20** (el quinto, java-s101; el cuarto fue el 17/09, 5 de 6 sólidos, flojo en sobrescritura). SÓLIDO: `main` es un método (una clase sin `main` compila y falla recién al EJECUTAR; `java Guitarra` corre aunque el archivo se llame Concierto.java) y que gana la versión de la clase del OBJETO. FLOJO: el ORDEN de la salida (escribió segundo lo que sale primero) y creer que "corre la del padre" cuando el padre NO declara ese método (ahí NO COMPILA: el compilador mira el tipo de la referencia). Sigue contestando media consigna cuando el enunciado viene en prosa y no numerado. Repasos: el de sobrescritura (20/09) actualizado + nuevo "compilador vs. JVM" al 23/09; ToDo/entregar-un-ejercicio.md ampliado (Niveles 2 y 3). Próximo examen: ~2026-10-04.
